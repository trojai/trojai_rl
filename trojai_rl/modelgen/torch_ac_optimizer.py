...
class TorchACOptimizer(RLOptimizerInterface):
    ...
    def test_env(self, env: gym.Env, model: nn.Module, max_steps: int, argmax_action: bool = False) \
            -> (list, int):
        """
        Test loop for one run of an agent through a single environment, i.e. from reset() to done=True.
        ...
        if model.recurrent:
            with torch.no_grad():
                ...
                return rewards, count, actions, info_dicts
        else:
            with torch.no_grad():
                obs = env.reset()
                obs = preprocess([obs], device=self.config.device)  # put in list to add batch dimension
    ...
    def _one_test(self, model: nn.Module, env_factory: EnvironmentFactory, test_cfg: TestConfig):
        ...
        for i in range(test_cfg.count):
            run_rewards, run_steps, run_actions, run_info_dicts = self.test_env(env, model, self.config.test_max_steps,
        ...
        return rewards, steps, actions, info_dicts, env
    ...
    def _test(self, model: nn.Module, env_factory: EnvironmentFactory, intermediate: bool = False) -> Any:
        ...
        for cfg in cfgs:
            rewards, steps, actions, info_dicts, env = self._one_test(model, env_factory, cfg)
